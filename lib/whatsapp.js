...
export function receiptPdfBuffer(receipt) {
  return new Promise((resolve, reject) => {
    ...
    // The artwork still has the previous domain set into it. Until the source
    // file is re-exported, cover that corner and print the live one, so every
    // receipt agrees with the website and the Google listing.
    doc.rect(468, 810, 120, 22).fill("#ffffff");
    doc.fillColor(BRAND_BLUE).font("Helvetica-Bold").fontSize(9.5)
      .text(business.website, 468, 817, { width: 116, align: "right" });
